...
While a few fundamental concepts of SLGP modeling are stated here for context, readers interested in a deeper understanding of SLGPs and their mathematical foundations are encouraged to refer to our main papers, [Insert Paper Title] [cite].
...
The posterior distribution of $\boldsymbol \varepsilon $, given data $\{(\xX_i, t_i)\}_{i=1}^n$ can be obtained by replacing Z's expression in Equation~\ref{eq:int_post_hyperpar} and denoting by $\phi_p$ the pdf of the $p$-variate standard normal distribution:
\[
...
\]
this formulation allows us to perform posterior inference, here maximum a posteriori (MAP) estimation.
...
With the synthetic data prepared, we now proceed to the estimation phase to evaluate the performance of SLGP-based models. This phase involves conducting inference experiments under various conditions to comprehensively assess the models' ability to recover the underlying distributions.
...
* Sample Sizes: we work with a range of sample sizes, varying from 5 to 250000 data points.

To ensure the robustness of our results and mitigate the impact of random seed variability, we perform 25 repetitions for each experimental setting. This comprehensive experimental design leads to a total of:
\[
...
By conducting these 30,000 experiments, we aim to thoroughly evaluate the posterior consistency and overall performance of SLGP-based models across diverse scenarios, varying in data sparsity, model smoothness, and basis function complexity.

For reproducibility, we provide the code used to perform the analyses. However, due to the independence of each experiment, we executed them in parallel on, UBELIX the high-performance computing cluster of the University of Bern.[TODO: citation]
...
dfres %>%
  ...
  summarise(q10=quantile(dH, probs=0.10),
            ...
            .groups="keep")%>%
  ggplot()+
  geom_line(mapping=aes(x=n, y=q50, col=as.factor(nFreq)))+
  geom_ribbon(mapping=aes(x=n, ymin=q10, ymax=q90, col=as.factor(nFreq), fill=as.factor(nFreq)),
              alpha=0.1, lty=2)+
...
for(ref in seq(4)){
  ...
  for(modid in seq(4)){
    ...
    plot <- dfplot %>%   
      ...
      ylab("Rescaled probability density")+
      xlab("Response variable t")+
      labs(title="Learning with a well-specified SLGP: MAP estimate of the density field for varying sample sizes",
           subtitle = paste0("Spatial regularity: ", 
                             c("Lowest", "Low", "High", "Highest"))[ref])
    ggsave(plot, file=paste0("./figures/ref", ref, 
                             "mod", modid, "nFreq", nFreq, ".png"), width = 10, height=4.5)
...
![Spatial regularity: lowest](./figures/ref1mod1nFreq500.png){ width=100% }
![Spatial regularity: lowest](./figures/ref2mod2nFreq500.png){ width=100% }
![Spatial regularity: lowest](./figures/ref3mod3nFreq500.png){ width=100% }
![Spatial regularity: lowest](./figures/ref4mod4nFreq500.png){ width=100% }
# Ill-specified case

![Spatial regularity: lowest](./figures/ref1mod2nFreq500.png){ width=100% }
