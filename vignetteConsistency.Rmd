...
```{r finalplot, fig.fullwidth=TRUE, fig.height=5, fig.width=10, fig.align='center' ,fig.pos="H"}
...
for(ref in seq(4)){
  dfplot <- dfGrid2
  ind <- c(1:nmax+(rep-1)*nmax)
  dfsamp <- data.frame()
  
  if(ref==1){
    dfplot$pdf_1 <- pred1$pdf_1
    refval <- pred1$pdf_1
    current_samp <- data.frame(t=samp$t1[ind], x=samp$x[ind])
  }
  if(ref==2){
    dfplot$pdf_1 <- pred2$pdf_1
    refval <- pred2$pdf_1
    current_samp <- data.frame(t=samp$t2[ind], x=samp$x[ind])
  }
  if(ref==3){
    dfplot$pdf_1 <-refval <- pred3$pdf_1
    current_samp <- data.frame(t=samp$t3[ind], x=samp$x[ind])
  }
  if(ref==4){
    dfplot$pdf_1 <-refval <- pred4$pdf_1
    current_samp <- data.frame(t=samp$t4[ind], x=samp$x[ind])
  }
  dfplot$name <- "Reference"
  modid <- ref
  for(n in c(50, 500, 50000)){
    title_file <- paste0("./res/SLGP_ref", ref, 
                         "_mod", modid, "_freq", nFreq, 
                         "_n", n, "_rep", rep, ".RData")
    if(file.exists(title_file)){
