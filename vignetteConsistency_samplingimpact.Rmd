...
for(i in seq(nRep)){
  for(nDiscret in c(51, 501)){
    cat(i, ".1\n")
    set.seed(i)
    x_candidates <- sample(seq(0, 1,, 51), nmax, replace=TRUE)
    set.seed(i)
    ...
    samp <- samp[shuffle, ]
    save(samp, file=paste0("./samplingstudy/SLGPsSamp", i, 
                           "discret", nDiscret, 
                           "case1.Rdata"))
  }
}
...
i<- 1
for(i in seq(nRep)){
  for(nDiscret in seq(51, 501)){
    cat(i, ".1\n")
    set.seed(i)
    x_candidates <- sample(seq(0, 1,, 501), nmax, replace=TRUE)
    set.seed(i)
    t <- as.data.frame(table(x_candidates))
    sampTemp <- sampleSLGP(SLGPmodel=SLGP1, 
                           interpolateBasisFun="WNN",
