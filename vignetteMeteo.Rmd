...
plotDensities <- merge(samples, stations[, c(2, 4, 5, 7)]) %>%
  ...
  geom_density(mapping=aes(x=Temperature), col="darkgrey")+
...
for(i in seq(nrow(stations))){
  ...
  Lin[[i]][["histogram"]] <-   samples%>%
    ...
    geom_density(mapping=aes(x=Temperature), col="chartreuse3")+
    scale_fill_manual(values=c("In the canton of Bern (test set)"="cornflowerblue", 
                               "Out of the canton of Bern (train set)"="darkgrey"))+
    facet_wrap(paste0(Station, " (", Altitude, "m)")~.)+
    theme_bw() +
...
We start by loading the predictions over Switzerland.
```{r eval=FALSE}
# Recombine chunks
data_chunks <- list()
...
df_plot_meteo3$Latitude <- (df_plot_meteo3$Latitude - range_lat[1])/diff(range_lat)
df_plot_meteo3$Longitude <- (df_plot_meteo3$Longitude - range_long[1])/diff(range_long)
# line y=-1.05 x+ 0.85
library(StereoMorph)
orth_vect <- orthogonalProjectionToLine(df_plot_meteo3[, c(1:2)], l1 = c(0, 0.85), 
                                        l2 = c(1, 0.85-1.05))
df_plot_meteo3$x1proj <- orth_vect[, 1]
df_plot_meteo3$x2proj <- orth_vect[, 2]
df_plot_meteo3$dproj <- sqrt((df_plot_meteo3$x1-df_plot_meteo3$x1proj)^2+
                               (df_plot_meteo3$x2-df_plot_meteo3$x2proj)^2)
...
df_plot_meteo2_summary <- df_plot_meteo3 %>%
...
stations_other <- subset(df_plot_meteo2_summary,
                         !is.na(df_plot_meteo2_summary$Station))
