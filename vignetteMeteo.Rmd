...
disc_long <- 201/diff(range(df$Longitude))
disc_lat <- 201/diff(range(df$Latitude))
df <- df %>%
...
for(i in seq(rep)){
  ...
  if(i%%5==0){
    cat("Saved just in case at ", i%/%5, "/47\n")
...
disc_long <- 101/diff(range(df$Longitude))
disc_lat <- 101/diff(range(df$Latitude))
df <- df %>%
  mutate(Longitude=round(Longitude*(disc_long-1))/(disc_long-1),
         Latitude=round(Latitude*(disc_lat-1))/(disc_lat-1))%>%
  group_by(Longitude, Latitude) %>%
...
for(i in seq(rep)){
  ...
  temp <- predictSLGP_newNode(SLGPmodel=model3,
                              newNodes = df2[1:n  + (i-1)*n, ],
                              nDiscret=201)
  dfSwissMCMC <- rbind(dfSwissMCMC, temp)
  ...
  if(i%%8==0){
    cat("Saved just in case at ", i%/%8, "/12\n")
...
This approach is intuitively meaningful when considering Switzerland's diverse geography:

  * *Cold Temperatures (e.g., -15°C):* The SLGP modelling predominantly identifies locations in the Alps and at high altitudes, where cold conditions are common due to elevation and geographical isolation. This aligns with our expectation, as the high Alpine summits and mountainous regions experience colder temperatures.

  * *Moderate Temperatures (e.g., 5°C):* The SLGP MAP estimator shows a broader range of possible locations, including the Swiss Plateau and parts of Ticino. Additionally, moderate temperatures are also observed in mountainous regions, provided they aren’t at very high altitudes.
