...
t <- as.numeric(df_plot_meteo4$Temperature[1:141])
value_cdf <- sapply(seq(4), function(i){
  print(i)
  tapply(df_plot_meteo4[, 4+i], INDEX=df_plot_meteo4$ID, function(pdf){
    cdf <-  cumsum(unlist(pdf))
    r <- range(cdf)
    cdf <- ((cdf-r[1])/(r[2]-r[1]))[, 1]
    # invcdf <- approx(x=cdf, y=t, xout=c(0.1, 0.5, 0.9))
    return(cdf)
  })
})
save(values, file="valuesOverSwitzerland.RData")
```

```{r}
load(file="valuesOverSwitzerland.RData")
df_plot_meteo4 <-  df_plot_meteo4 %>%
  dplyr::select(-subset1, -subset2, -Station)%>%
  pivot_longer(-c("Latitude", "Longitude", "Altitude", "Temperature"))%>%
  mutate(simu=substr(name, 5, 11),
         valuepdf=value) %>%
  group_by(Latitude, Longitude, Altitude, simu)%>%
  arrange(Temperature) %>%   # Sort by ascending Temperature within groups
  ...
  dplyr::select(-c("name", "value"))%>%
  group_by(Latitude, Longitude, Altitude, simu) %>%
  summarise(meanT = sum(Temperature*valuepdf)/sum(valuepdf),
            q10 = (Temperature[which.min(abs(valuecdf-0.1))])[1],
            q50 = (Temperature[which.min(abs(valuecdf-0.5))])[1],
            q90 = (Temperature[which.min(abs(valuecdf-0.9))])[1],
            .groups="keep")%>%
  ungroup()%>%
  pivot_longer(-c("Latitude", "Longitude", "Altitude", "simu"))%>%
  group_by(Latitude, Longitude, Altitude, name) %>%
