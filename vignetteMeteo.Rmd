...
```{r plotInvPb, fig.cap = "An inverse problem solved with SLGP modelling: temeprature based localisation", fig.fullwidth=TRUE, fig.height=4.0, fig.width=9.5, fig.align='center',fig.pos="H", warning=FALSE, message=FALSE, echo=FALSE}
...
```{r plotSlicesDrawn, fig.cap = "Two slices across Switzerland", fig.fullwidth=TRUE, fig.height=6.0, fig.width=8, fig.align='center',fig.pos="H", warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
disc_long <- 101
disc_lat <- 101
...
We visualise the topographic profile of the first slice with the location of the stations alongside it, as well as our probabilistic joint prediction of the Temperature quantiles along this slice.
...
plot2 <- 
  ...
  geom_label(data=stations_other, mapping=aes(label=Station, 
                                              y=27+c(0, -4.5, 0, 0), x=place_on_line), inherit.aes = FALSE)

library(ggpubr)
plot_grid <- ggarrange(plotmap1+coord_fixed()+ggtitle("North West to South East"), plot1, plot2,
...
We do the same for the second slice.


```{r include=FALSE}
# Extract the chunks on the slice and project them in 1D
df_plot_meteo3 <- subset(df_plot_meteo2, df_plot_meteo2$subset2)
df_plot_meteo3$x2 <- (df_plot_meteo3$Latitude - range_lat[1])/diff(range_lat)
df_plot_meteo3$x1 <- (df_plot_meteo3$Longitude - range_long[1])/diff(range_long)
library(StereoMorph)
orth_vect <- orthogonalProjectionToLine(df_plot_meteo3[, c("x1", "x2")], 
                                        l1 = c(0, intercept2), 
                                        l2 = c(1, -slope2+intercept2))
df_plot_meteo3$x1proj <- orth_vect[, 1]
df_plot_meteo3$x2proj <- orth_vect[, 2]
...
df_plot_meteo3$place_on_line <- apply(df_plot_meteo3[, c("x1proj", "x2proj")], 
                                      1, function(u) {
                                        return(sum((u - c(0, intercept2)) * dir2))
                                      })
plot(df_plot_meteo3$x1, df_plot_meteo3$place_on_line)

r2 <- range(df_plot_meteo3$place_on_line)
df_plot_meteo3$place_on_line <- (df_plot_meteo3$place_on_line-r2[1])/diff(r2)
...
df_plot_meteo_summary_2 <- df_plot_meteo3 %>%
  group_by(place_on_line, simu)%>%
  summarise(Altitude=mean(Altitude),
            mean = mean(Temperature*valuepdf),
            Station = ifelse(all(is.na(Station)), NA, unique(na.omit(Station))),
...
stations_other <- subset(df_plot_meteo_summary_2,
                         !is.na(df_plot_meteo_summary_2$Station))
stations_other <- subset(stations_other,
                         stations_other$simu==1)
```
...
library(ggpubr)
plot_grid <- ggarrange(plotmap2+coord_fixed()+ggtitle("North West to South East"), plot1, plot2,
                          ncol = 1, nrow = 3,align = "v", 
                          heights = c(2, 2, 4))
plot(plot_grid)
ggsave(plot_grid, file="./Figures/slice_switzerland2.png",
       width=8.5, height=7., scale=1.1)
ggsave(plot_grid, file="./Figures/slice_switzerland2.pdf",
       width=8.5, height=7., scale=1.1)
```
