...
```{r, fig.fullwidth=TRUE, fig.height=3, fig.width=6, fig.align="center", fig.cap = "Unconditional simulations of GPs with our two kernels of interest", fig.pos="H", warning=FALSE, message=FALSE}
u <- seq(0, 1,, 51)
...
dfGrid %>%
  pivot_longer(-c("x", "t"))%>%
  ggplot()+
  geom_raster(mapping=aes(x=x, y=t, fill=value), interpolate=TRUE)+
  facet_grid(.~name)+ 
...
```{r, fig.fullwidth=TRUE, fig.height=4,fig.align="center",  fig.cap = "Unconditional simulations of SLGPs with our two kernels of interest", fig.pos="H", warning=FALSE, message=FALSE}
scale_factor <- 0.05
...
dfGrid %>%
  ...
  mutate(SLGP=exp(GP-max(GP))/mean(exp(GP-max(GP))),
         name=paste0("SL", name))%>%
  ggplot(aes(x=t, ymax=scale_factor*SLGP+x, ymin=x, group=-x, fill=x))+
...
```{r, fig.fullwidth=TRUE, fig.height=2,fig.align="center", fig.cap = "The x values used, with smaller step sizes around zero (log-scale).", fig.pos="H", warning=FALSE, message=FALSE}
...
```{r fig.cap="Empirical and theoretical rates, as estimated on unconditional simulations", fig.fullwidth=TRUE, fig.align="center", fig.height=5, fig.width=7, fig.pos="H", warning=FALSE, message=FALSE}
max_x <- 0.01
...
df_plot%>%
  ...
  scale_linewidth_manual( values = c("Empirical rates"=0.5,
                                     "Theoretical rates"=1.25) ) +
  scale_linetype_manual(values = c("Empirical rates"="solid",
                                   "Theoretical rates"="dashed"))+
